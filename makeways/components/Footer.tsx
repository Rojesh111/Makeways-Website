...
/*
  Font family roles:
    'EurostileExt'  — MAKEWAYS wordmark, SOCIALS heading (large display)
    'EurostileCnd'  — CONTACT DETAIL label, nav links, toggle buttons, copyright, badges
    'Eurostile'     — contact detail text (body), map CTA

  NO font-weight: 900 — 700 = bold, 400 = regular.
  Color token: #f47c20 (was #F5A623 — now unified across all components).
*/
  ...
  return (
    ...
    >
      ...
      <style jsx>{`
        ...
        /* FIX — hover color #f47c20, was #F5A623 */
        .sb.hov { background: #f47c20; color: #fff; transform: translateY(-3px); }
      `}</style>
    </a>
  );
}
...
function ContactRow({ Icon, text, href }: { Icon: () => JSX.Element; text: string; href?: string }) {
  ...
  if (href) {
    return (
      <a href={href} style={{ textDecoration: 'none' }} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}>
        {inner}
      </a>
    );
  }
  ...
  return (
    <>
      ...
      <footer className="ft" id="contact">
        ...
        <div className="ft__main-row">
          ...
          <div className="ft__left">
            ...
            {/* EurostileExt Bold — wordmark display title */}
            <h2 className="ft__wordmark">MAKEWAYS</h2>

            {/* EurostileCnd Bold — overline label */}
          ...
          <div className="ft__right">
            ...
            <div className="ft__socials-row">
              {/* EurostileExt Bold — SOCIALS display heading */}
            ...
            <div className="ft__map-card">
              ...
              <div className="ft__toggle">
                {(['street', 'satellite'] as const).map(v => (
                  <button
                    key={v}
                    className={`ft__tog${mapView === v ? ' active' : ''}`}
      ...
      <style jsx>{`
        ...
        .ft__toggle { display: flex; border-bottom: 1px solid #ddd; }

        /* EurostileCnd Bold — toggle button labels */
        .ft__tog {
          flex          : 1;
          padding       : 9px 0;
          background    : #f5f5f5;
        ...
        .ft__iframe-wrap { height: 200px; flex-shrink: 0; }

        /* Eurostile Bold — map CTA */
        .ft__map-cta {
          display        : block;
          text-align     : center;
          padding        : 10px 16px;
        ...
        .ft__map-cta:hover { background: #f47c20; color: #fff; }
